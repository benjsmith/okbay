"""Keep okstratr desk kernel alive for okbay (core skill).

Mirrors Switchbay ``okstratr_supervisor``: start on loopback (default
``http://127.0.0.1:8767``, override ``OKBAY_OKSTRATR_UPSTREAM``), health-check
``/health``, restart on death. Pid/log under ``~/.local/state/okbay/``.

Spawn: ``okstratr serve --host/--port`` on PATH (or ``OKBAY_OKSTRATR_BIN`` /
``python -m okstratr serve``). Sets ``OKSTRATR_HOST=okbay`` so hosted mode
(Herdr notify + HTML settings off) applies.
"""
...
log = logging.getLogger("okbay.okstratr_supervisor")
...
def upstream_base() -> str:
    raw = (os.environ.get("OKBAY_OKSTRATR_UPSTREAM") or "").strip()
...
def health_url() -> str:
    return upstream_base() + _HEALTH_PATH
...
def resolve_okstratr_argv() -> list[str]:
    ...
    override = (os.environ.get("OKBAY_OKSTRATR_BIN") or "").strip()
    if override:
        return [override]
...
def start(workspace: Path | None = None) -> dict[str, Any]:
    ...
    env = os.environ.copy()
    env.setdefault("OKSTRATR_HOST", "okbay")
    env.setdefault("OKSTRATR_HOSTED", "okbay")
    # Default notify URL → okbay Herdr stub (contract C2).
    env.setdefault(
        "OKSTRATR_HOST_NOTIFY_URL",
        "http://127.0.0.1:8766/api/okstratr/host-notify",
    )
...
def run_supervisor_once(workspace: Path | None = None) -> dict[str, Any]:
    """One keep-alive tick (thread-friendly)."""
    if is_healthy():
...
def poll_forever(workspace: Path | None = None, *, interval: float = _POLL_SEC) -> None:
    """Blocking keep-alive loop (run in a daemon thread)."""
    while True:
        try:
            run_supervisor_once(workspace)
